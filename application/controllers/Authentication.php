...
class Authentication extends CRM_Controller
{
    public function __construct()
    {
        ...
        $this->load->library('QRAuthenticator');
    ...
    //new add by sakhideveloper@gmail.com from pakistan
    public function enable2FA(){
      if($this->input->post()){
           $code = $this->input->post('code');
           $sercret_key = $this->session->userdata('google_auth_secret');
           if($this->qrauthenticator->verifyCode($sercret_key, $code)){
            if($this->input->post('authenticate') && $this->input->post('authenticate') == 'on'){
             $data['two_factor_auth_enabled']=1;
            }else{
             $data['two_factor_auth_enabled']=0;
            }
            $data['two_factor_auth_code']=$sercret_key;
            if($this->session->userdata('staff_user_id'))
            $this->db->update('tblstaff',$data,array('staffid'=>$this->session->userdata('staff_user_id')));
            if($this->session->userdata('client_user_id'))
            $this->db->update('tblstaff',$data,array('staffid'=>$this->session->userdata('client_user_id')));
      ...
      $sercret_key = $this->qrauthenticator->createSecret();
      $this->session->set_userdata('google_auth_secret',$sercret_key);
      $data['qr_code_url'] = $this->qrauthenticator->getURL($sercret_key);
      $data['sercret_key']=$sercret_key;
      $data['title'] = 'two factor authentication';
      $this->load->view('authentication/two_factor_auth', $data);
     }
    public function admin()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                $two_factor=$this->input->post('two_factor_auth');
                $allowed=$this->session->userdata('two_factor_auth_enabled');
                ...
                }elseif(isset($two_factor) || $allowed==1){
                    redirect(site_url('authentication/enable2FA'));  
