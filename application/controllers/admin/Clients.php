...
class Clients extends Admin_controller
{
    ...
    public function index()
    {
        ...
        // $update='{"aside_menu_active":[{"name":"als_dashboard","url":"\/","permission":"","icon":"fa fa-tachometer","id":"dashboard"},{"name":"als_activity_log_submenu","url":"utilities\/activity_log","permission":"is_admin","icon":"","id":"child-activity-log"},{"name":"als_calendar_submenu","url":"utilities\/calendar","permission":"","icon":"","id":"child-calendar"},{"name":"subscriptions","permission":"subscriptions","icon":"fa fa-repeat","url":"subscriptions","id":"subscriptions"},{"name":"als_clients","url":"clients","permission":"customers","icon":"fa fa-users","id":"client"},{"name":"als_customer","url":"customers","permission":"customers","icon":"fa fa-user-circle-o","id":"customers"},{"name":"projects","url":"projects","permission":"","icon":"fa fa-bars","id":"projects"},{"name":"als_tasks","url":"tasks\/list_tasks","permission":"","icon":"fa fa-tasks","id":"tasks"},{"name":"als_leads","url":"leads","permission":"is_staff_member","icon":"fa fa-tty","id":"leads"},{"name":"als_sales","url":"#","permission":"","icon":"fa fa-balance-scale","id":"sales","children":[{"name":"invoices","url":"invoices\/list_invoices","permission":"invoices","icon":"","id":"child-invoices"},{"name":"payments","url":"payments","permission":"payments","icon":"","id":"child-payments"},{"name":"proposals","url":"proposals","permission":"proposals","icon":"","id":"child-proposals"},{"name":"estimates","url":"estimates\/list_estimates","permission":"estimates","icon":"","id":"child-estimates"},{"name":"items","url":"invoice_items","permission":"items","icon":"","id":"child-items"}]},{"name":"als_announcements_submenu","url":"announcements","permission":"is_admin","icon":"","id":"child-announcements"},{"name":"support","url":"tickets","permission":"","icon":"fa fa-ticket","id":"tickets"},{"name":"als_expenses","url":"expenses\/list_expenses","permission":"expenses","icon":"fa fa-heartbeat","id":"expenses"},{"name":"als_media","url":"utilities\/media","permission":"","icon":"","id":"child-media"},{"name":"als_kb","url":"#","permission":"knowledge_base","icon":"fa fa-folder-open-o","id":"knowledge-base","children":[{"name":"als_all_articles","url":"knowledge_base","permission":"","icon":"","id":"child-all-articles"},{"name":"als_kb_groups","url":"knowledge_base\/manage_groups","permission":"","icon":"","id":"child-groups"}]},{"name":"als_utilities","url":"#","permission":"","icon":"fa fa-cogs","id":"utilities","children":[{"name":"bulk_pdf_exporter","url":"utilities\/bulk_pdf_exporter","permission":"bulk_pdf_exporter","icon":"","id":"child-bulk-pdf-exporter"},{"name":"als_goals_tracking","url":"goals","permission":"goals","icon":"","id":"child-goals-tracking"},{"name":"als_surveys","url":"surveys","permission":"surveys","icon":"","id":"child-surveys"},{"name":"als_contracts","url":"contracts","permission":"contracts","icon":"fa fa-file","id":"contracts"},{"name":"utility_backup","url":"utilities\/backup","permission":"is_admin","icon":"","id":"child-database-backup"},{"name":"ticket_pipe_log","url":"utilities\/pipe_log","permission":"is_admin","icon":"","id":"ticket-pipe-log"}]},{"name":"als_reports","url":"#","permission":"reports","icon":"fa fa-area-chart","id":"reports","children":[{"name":"als_reports_sales_submenu","url":"reports\/sales","permission":"","icon":"","id":"child-sales"},{"name":"als_reports_expenses","url":"reports\/expenses","permission":"","icon":"","id":"child-expenses"},{"name":"als_expenses_vs_income","url":"reports\/expenses_vs_income","permission":"","icon":"","id":"child-expenses-vs-income"},{"name":"als_reports_leads_submenu","url":"reports\/leads","permission":"","icon":"","id":"child-leads"},{"name":"als_kb_articles_submenu","url":"reports\/knowledge_base_articles","permission":"","icon":"","id":"child-kb-articles"}]},{"name":"als_taskstemplates","url":"taskstemplates","permission":"is_staff_member","icon":"fa fa-tasks","id":"taskstemplates"}]}';
        // $value = array('value' => $update );
        // // $this->db->where('name','aside_menu_active');
        // $this->db->update('tbloptions',$value);
    ...
    {
        if (!is_admin()) {
            access_denied('Customer Seminars');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('customers_seminars');
        }
        $data['total_hours_years']=$this->clients_model->seminars_total_hours_years();
        $data['total_hours_year']=$this->clients_model->seminars_total_hours_year();
        $data['total_hours_month']=$this->clients_model->seminars_total_hours_month();
        $data['total_hours_day']=$this->clients_model->seminars_total_hours_day();
        $data['title'] = 'Seminars';
        $this->load->view('admin/clients/seminar_manage', $data);
    }
    ...
    public function seminar()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($data['id'] == '') {
                $id      = $this->clients_model->add_seminar($data);
                $message = $id ? _l('added_successfully', _l('customer_seminar')) : '';
                echo json_encode([
                ...
                $success = $this->clients_model->edit_seminar($data);
                $message = '';
                if ($success == true) {
                    $message = _l('updated_successfully', _l('customer_seminar'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
    ...
    public function delete_seminar($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('clients/seminars'));
        }
        $response = $this->clients_model->delete_seminar($id);
        if ($response == true) {
