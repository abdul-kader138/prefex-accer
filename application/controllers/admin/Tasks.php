...
class Tasks extends Admin_controller
{
    ...
    public function table_due_date()
    {
        $this->app->get_table_data('tasks_due_date');
    }
    ...
    /* List all tasks */
    public function list_due_date($id = '')
    {
        close_setup_menu();
        // If passed from url
        $data['custom_view'] = $this->input->get('custom_view') ? $this->input->get('custom_view') : '';
        ...
        $data['title'] = _l('tasks');
        $this->load->view('admin/tasks/manage_due_date', $data);
    }


    /* Add new task or update existing */
    public function task_due_date($id = '')
    {
        if (!has_permission('tasks', '', 'edit') && !has_permission('tasks', '', 'create')) {
            access_denied('Tasks');
        }
