...
class Contacts_model extends CRM_Model
{
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array('tblcontacts')) . ',' . get_sql_select_client_company());
    ...
    */
    ...
    public function require_confirmation($client_id)
    {
        ...
        $this->db->update('tblcontacts', ['active' => 0, 'registration_confirmed' => 0]);
    ...
    public function confirm_registration($client_id)
    {
        $contact_id = get_primary_contact_user_id($client_id);
        $this->db->where('userid', $client_id);
        $this->db->update('tblcontacts', ['active' => 1, 'registration_confirmed' => 1]);
    ...
    {
        //return $this->db->query('SELECT DISTINCT(country_id), short_name FROM tblcontacts JOIN tblcountries ON tblcountries.country_id=tblcontacts.country')->result_array();
    }
