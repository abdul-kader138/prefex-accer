...
class Clients_model extends CRM_Model
{
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete('tblcostomerseminar');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('groupid', $id);
            $this->db->delete('tblcostomerseminar');
            logActivity('Customer Seminar Deleted [ID:' . $id . ']');
     ...
     * @return boolean
     */
    public function seminars_total_hours_years()
    {
        $result=$this->db->query("SELECT hours_total FROM tblcostomerseminar")->result_array();
        return $result;
    } 
    public function seminars_total_hours_year()
    {
        $year=date("Y");
        $result=$this->db->query("SELECT hours_total FROM tblcostomerseminar WHERE YEAR(date) = $year")->result_array();
        return $result;
    }
    public function seminars_total_hours_month()
    {
        $m=date("m");
        $result=$this->db->query("SELECT hours_total FROM tblcostomerseminar WHERE MONTH(date) = $m")->result_array();
        return $result;
    }
    public function seminars_total_hours_day()
    {
        $date=date("Y-m-d");
        $result=$this->db->query("SELECT hours_total FROM tblcostomerseminar WHERE date = $date")->result_array();
        return $result;
    }
     ...
     * @param array $data $_POST data
     */
    public function add_seminar($data)
    {   
        $this->db->insert('tblcostomerseminar', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            logActivity('New Customer Seminar Created [ID:' . $insert_id . ', Name:' . $data['name'] . ']');
    ...
    public function edit_seminar($data)
    {
        ...
        $this->db->update('tblcostomerseminar', [
            'name' => $data['name'],
        ...
        if ($this->db->affected_rows() > 0) {
            logActivity('Customer Seminar Updated [ID:' . $data['id'] . ']');
