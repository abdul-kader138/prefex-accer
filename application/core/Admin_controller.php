...
class Admin_controller extends CRM_Controller
{
    ...
    public function __construct()
    {
        ...
        if (CI_VERSION != '3.1.9') {
            ...
            echo '<p>From the newest downloaded files upload the <b>system</b> folder to your Accerp CRM installation directory.';
            die;
        }
        ...
        if (!$is_ajax_request) {
            if (ENVIRONMENT == 'production' && is_admin()) {
                if ($this->config->item('encryption_key') === '') {
                    die('<h1>Encryption key not sent in application/config/config.php</h1>For more info google Encryption key ');
                } elseif (strlen($this->config->item('encryption_key')) != 32) {
                    die('<h1>Encryption key length should be 32 charachters</h1>For more info google encryption keys ');
                }
            }
